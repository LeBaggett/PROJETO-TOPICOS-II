...
void LCD_PCD8544_init(LCD_PCD8544_screen_t* scr)
{
    ...
    unsigned char addressing_type = 0; // horizontal
    ...
    LCD_PCD8544_LL_set_display_control(scr, 1, 0); // Normal mode
...
void LCD_PCD8544_draw_bitmap(
    LCD_PCD8544_screen_t* scr,
    const uint8_t* bmp
)
{
    uint8_t line[84];
    ...
    for (uint8_t page = 0; page < 6; page++)
    {
        ...
        {
            line[x] = reverse_byte(bmp[page * 84 + x]);
        }

        LCD_PCD8544_write_bytes(scr, page, 0, line, 84);
    }
}
